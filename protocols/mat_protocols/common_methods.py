...
def mixing(pipette, amount, rep, well, aspirate_speed=150, dispense_speed=300, height=0.6): #Default for robot is 150 and 300
    """Method to mix liquid in WELL by AMOUNT"""
    pipette.flow_rate.aspirate = aspirate_speed
    pipette.flow_rate.dispense = dispense_speed
    loc1 = well.bottom().move(types.Point(x=1, y =0, z=height))
    for i in range(rep):
        pipette.aspirate(amount, loc1)
        pipette.dispense(amount,loc1)
